import cv2
import numpy as np
import os
from pathlib import Path

def convertir_tiff_a_png(imagen_tiff, ruta_salida_png):
    """
    Convierte una imagen TIFF a PNG escalando para mejor visualización
    """
    # Escalar a 0-255 para mejor visualización en PNG
    img_min = imagen_tiff.min()
    img_max = imagen_tiff.max()
    
    if img_max > img_min:
        imagen_escalada = ((imagen_tiff - img_min) / (img_max - img_min) * 255).astype(np.uint8)
    else:
        imagen_escalada = imagen_tiff.astype(np.uint8)
    
    # Guardar como PNG
    cv2.imwrite(ruta_salida_png, imagen_escalada)

def procesar_imagenes_flares():
    """
    Procesa todas las imágenes aplicando resta de fondo y guarda SOLO PNG
    """
    
    # Rutas base
    ruta_base = r"C:\Users\resendizjg\Downloads\flares\tiffs"
    #ruta_base = r"C:\Users\resendizjg\Downloads\flares\tiffs\quiet"
    imagen_fondo_131 = r"C:\Users\resendizjg\Downloads\quite_average-131.tif"
    imagen_fondo_193 = r"C:\Users\resendizjg\Downloads\quite_average-193.tif"
    
    # Verificar que existan las imágenes de fondo
    if not os.path.exists(imagen_fondo_131):
        print(f"ERROR: No se encuentra la imagen de fondo 131: {imagen_fondo_131}")
        return
    
    if not os.path.exists(imagen_fondo_193):
        print(f"ERROR: No se encuentra la imagen de fondo 193: {imagen_fondo_193}")
        return
    
    # Cargar las imágenes de fondo
    print("Cargando imágenes de fondo...")
    fondo_131 = cv2.imread(imagen_fondo_131, cv2.IMREAD_UNCHANGED)
    fondo_193 = cv2.imread(imagen_fondo_193, cv2.IMREAD_UNCHANGED)
    
    if fondo_131 is None or fondo_193 is None:
        print("ERROR: No se pudieron cargar las imágenes de fondo")
        return
    
    print(f"Fondo 131: {fondo_131.shape}")
    print(f"Fondo 193: {fondo_193.shape}")
    
    # Contadores
    total_procesadas = 0
    procesadas_131 = 0
    procesadas_193 = 0
    errores = 0
    
    # Procesar cada carpeta
    for carpeta_num in os.listdir(ruta_base):
        carpeta_path = os.path.join(ruta_base, carpeta_num)
        
        if not os.path.isdir(carpeta_path):
            continue
            
        during_path = os.path.join(carpeta_path, "during")
        
        if not os.path.exists(during_path):
            continue
        
        print(f"\nProcesando: {carpeta_num}/during/")
        
        # CREAR SOLO LA CARPETA diferencia_png
        png_path = os.path.join(carpeta_path, "diferencia_png")
        os.makedirs(png_path, exist_ok=True)
        print(f"Salida PNG: {carpeta_num}/diferencia_png/")
        
        # Procesar archivos
        for archivo in os.listdir(during_path):
            archivo_path = os.path.join(during_path, archivo)
            
            # Procesar archivos _0131.tiff
            if archivo.lower().endswith('_0131.tiff'):
                try:
                    imagen = cv2.imread(archivo_path, cv2.IMREAD_UNCHANGED)
                    
                    if imagen is None:
                        errores += 1
                        continue
                    
                    if imagen.shape != fondo_131.shape:
                        print(f"  ERROR dimensiones: {archivo}")
                        errores += 1
                        continue
                    
                    # Resta
                    diferencia = cv2.subtract(imagen, fondo_131)
                    
                    # Guardar SOLO PNG
                    nombre_salida = archivo.replace('.tiff', '_diff.png')
                    ruta_salida = os.path.join(png_path, nombre_salida)
                    convertir_tiff_a_png(diferencia, ruta_salida)
                    
                    print(f"  ✓ {nombre_salida}")
                    procesadas_131 += 1
                    total_procesadas += 1
                    
                except Exception as e:
                    print(f"  ERROR: {archivo} - {str(e)}")
                    errores += 1
                    
            # Procesar archivos _0193.tiff
            elif archivo.lower().endswith('_0193.tiff'):
                try:
                    imagen = cv2.imread(archivo_path, cv2.IMREAD_UNCHANGED)
                    
                    if imagen is None:
                        errores += 1
                        continue
                    
                    if imagen.shape != fondo_193.shape:
                        print(f"  ERROR dimensiones: {archivo}")
                        errores += 1
                        continue
                    
                    # Resta
                    diferencia = cv2.subtract(imagen, fondo_193)
                    
                    # Guardar SOLO PNG
                    nombre_salida = archivo.replace('.tiff', '_diff.png')
                    ruta_salida = os.path.join(png_path, nombre_salida)
                    convertir_tiff_a_png(diferencia, ruta_salida)
                    
                    print(f"  ✓ {nombre_salida}")
                    procesadas_193 += 1
                    total_procesadas += 1
                    
                except Exception as e:
                    print(f"  ERROR: {archivo} - {str(e)}")
                    errores += 1
    
    # Resumen
    print("\n" + "="*60)
    print("RESUMEN")
    print("="*60)
    print(f"Total procesadas: {total_procesadas}")
    print(f"  - _0131.tiff: {procesadas_131}")
    print(f"  - _0193.tiff: {procesadas_193}")
    print(f"Errores: {errores}")
    print("="*60)
    
    if total_procesadas > 0:
        print("\n✓ Completado!")
        print("Los PNG están en las carpetas 'diferencia_png'")

if __name__ == "__main__":
    print("="*60)
    print("PROCESADOR DE FLARES SOLARES - SALIDA PNG")
    print("="*60)
    
    print("\nEste script:")
    print("1. Lee las imágenes TIFF de las carpetas XX/during/")
    print("2. Aplica resta de fondo (131 o 193 según corresponda)")
    print("3. Guarda SOLO PNG en las carpetas XX/diferencia_png/")
    
    respuesta = input("\n¿Proceder? (s/n): ").strip().lower()
    
    if respuesta == 's':
        procesar_imagenes_flares()
    else:
        print("Cancelado.")
